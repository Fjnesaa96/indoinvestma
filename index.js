...
app.post('/api/register', async (req, res) => {
  ...
  if (String(password).trim().length < 6) return res.status(400).json({ success: false, message: 'Sandi minimal 6 karakter.' });
  if (!supabase) return res.status(500).json({ success: false, message: 'Database Supabase belum terhubung.' });

  try {
    const formattedPhone = formatPhone(phone);
    const rawTail = cleanDigits(phone).slice(-9);

    const { data: existing } = await supabase
      .from('users')
      .select('id, phone')
      .or('phone.eq.' + formattedPhone + ',phone.ilike.%' + rawTail)
      .limit(1);
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    const { data: userList, error: fetchErr } = await supabase
      .from('users')
      .select('*')
      .or('phone.eq.' + formattedPhone + ',phone.ilike.%' + rawTail)
      .limit(1);
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        userReferralCode: target.user_referral_code,
...
app.get('/api/user/sync', async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ success: true, balance: Number(user ? user.balance : 0), investments: invs || [] });
  } catch (err) {
...
app.get('/api/referrals', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ success: false, message: 'Gagal memuat referral.' });
  }
});
...
app.post('/api/deposit', async (req, res) => {
  ...
  try {
    const { data: user } = await supabase.from('users').select('balance').eq('id', userId).single();
    const newBal = Number(user ? user.balance : 0) + num;
...
app.post('/api/invest', async (req, res) => {
  ...
  try {
    const { data: user } = await supabase.from('users').select('balance').eq('id', userId).single();
    const current = Number(user ? user.balance : 0);
...
app.post('/api/withdraw', async (req, res) => {
  ...
  if (!userId || !bankName || !accountNumber || !accountHolder || isNaN(num) || num < 50000 || !supabase) {
    return res.status(400).json({ success: false, message: 'Data tidak lengkap / Minimal penarikan Rp 50.000.' });
  }
  ...
  try {
    ...
    const current = Number(user ? user.balance : 0);
    if (current < num) {
      return res.status(400).json({ success: false, message: 'Saldo tidak cukup (Rp ' + current.toLocaleString('id-ID') + ').' });
    }
    ...
    return res.status(200).json({
      success: true,
      message: 'Penarikan Rp ' + num.toLocaleString('id-ID') + ' diajukan! Biaya admin 10%: Rp ' + fee.toLocaleString('id-ID') + ', bersih: Rp ' + net.toLocaleString('id-ID') + '.',
      newBalance: newBal
    });
  } catch (err) {
...
<body class="text-white flex justify-center min-h-screen">
  ...
  <script>
    ...
    function cleanDigitsClient(val) {
      return String(val || '').replace(/[^0-9]/g, '');
    }

    function formatPhoneClient(val) {
      var d = cleanDigitsClient(val);
      if (d.startsWith('62')) d = d.substring(2);
      if (d.startsWith('0')) d = d.substring(1);
      return '+62' + d;
    }
    ...
    function showFeedback(msg, isSuccess) {
      feedbackBox.className = 'text-xs px-4 py-3 rounded-xl font-medium text-center ' +
    ...
    function setAuthMode(isRegister) {
      ...
      if (isRegister) {
        tabRegister.className = 'flex-1 py-2 text-xs font-semibold rounded-xl bg-[#2c2c2e] text-white shadow-sm transition';
        tabLogin.className = 'flex-1 py-2 text-xs font-semibold rounded-xl text-neutral-400 transition';
        referralRow.classList.remove('hidden');
        btnText.textContent = 'Daftar Sekarang';
        switchPrompt.innerHTML = 'Sudah punya akun? <span class="text-amber-400 font-bold underline">Masuk di sini</span>';
      } else {
        tabLogin.className = 'flex-1 py-2 text-xs font-semibold rounded-xl bg-[#2c2c2e] text-white shadow-sm transition';
        tabRegister.className = 'flex-1 py-2 text-xs font-semibold rounded-xl text-neutral-400 transition';
        referralRow.classList.add('hidden');
        btnText.textContent = 'Masuk ke Akun';
        switchPrompt.innerHTML = 'Belum punya akun? <span class="text-amber-400 font-bold underline">Daftar di sini</span>';
      }
    ...
    tabRegister.onclick = function() { setAuthMode(true); };
    tabLogin.onclick = function() { setAuthMode(false); };
    switchPrompt.onclick = function() { setAuthMode(!isRegisterMode); };
    ...
    authForm.onsubmit = async function(e) {
      ...
      if (!raw) {
        showFeedback('Nomor handphone wajib diisi.', false);
        return;
      }

      var phone = formatPhoneClient(raw);
      var password = passwordInput.value.trim();
      var shareCode = shareCodeInput.value.trim();

      if (password.length < 6) {
        showFeedback('Kata sandi minimal 6 karakter.', false);
        return;
      }
      ...
      try {
        ...
        if (!res.ok) throw new Error(result.message || 'Gagal terhubung ke server.');
    ...
    async function loadReferrals(code) {
      if (!code) return;
      try {
        var res = await fetch('/api/referrals?referralCode=' + encodeURIComponent(code));
        var result = await res.json();
